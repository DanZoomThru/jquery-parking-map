	...
	var packages;
		base._getListings = function (callback, eventCall) {
			var venues = base.options.location.venue,
				...
				default_event = null,
				i = 0;
			packages = base.options.location.packages,
			base.listings = [];
			...
			$.each(search, function (index, searchQuery) {
				$.ajax('//api.parkwhiz.com/' + searchQuery.uri, {
					...
					success: function(searchResults) {
						i++;
						if (packages !== null && packages !== undefined) {
							base.$el.find('.parking_locations-mod h2').text('Parking Packages for ' + searchResults.venue_name);
						}
			...
			if (!_.isEmpty(listings)) {
				...
				for (var i = 0, listingsLoop = listings.length; i < listingsLoop; i++) {
					...
					if (item.price) {
						...
						if (packages !== null && packages !== undefined) {
							packageParam = 'package=' + item.listing_id;
						} else {
							packageParam = '';
						}
						if (_.contains(base.options.modules, 'parking_locations')) {
							...
							if(item.available_spots <= 0) {
								$parkingLocationsDiv.animate({height:175},600).append('<li><a href="'+item.parkwhiz_url+packageParam+'" target="_blank" data-lat="'+item.lat+'" data-lng="'+item.lng+'" data-listing_id="'+item.listing_id+'" title="Parking for '+item.location_name+'">'+item.address+' <em>(Sold Out)</em></a></li>');
							} else {
								$parkingLocationsDiv.animate({height:175},600).append('<li><a href="'+item.parkwhiz_url+packageParam+'" target="_blank" data-lat="'+item.lat+'" data-lng="'+item.lng+'" data-listing_id="'+item.listing_id+'" title="'+item.price_formatted+' Parking for '+item.location_name+'">'+item.address+' <em>('+item.price_formatted+')</em></a></li>');
							}
						}
					}
				}
					...
					marker: {
						...
						events: {
							...
							click: function (marker, event, context) {
								if (context.data && context.data.listing.parkwhiz_url+packageParam) {
									window.open(context.data.listing.parkwhiz_url+packageParam);
								}
							}
						}
					}
